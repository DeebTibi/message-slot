...
static ssize_t device_read(struct file* file, char __user* buffer, size_t length, loff_t* offset){
    ...
    char* channel_buffer = channeln->data->buffer;
    if(channel_buffer == NULL){
        return -EWOULDBLOCK;
    }
    size_t buffer_ln = channeln->data->buffer_len;
    if(buffer_ln > length){
        return -ENOSPC;
    }
    size_t message_length = channeln->data->message_length;
    ssize_t bytes_read = 0;
    for(int i = 0; i < message_length; i++, bytes_read++){
        // put user can fail, need to see what is the intened behavior if this fails.
...
static ssize_t device_write(struct file* file, const char __user* buffer, size_t length, loff_t* offset){
    ...
    for(int i = 0; i < length; i++, bytes_written++){
...
static long int device_ioctl(struct file* file, unsigned int cmd, unsigned long param){
    ...
    channel_node* channeln = get_channel(msg_slot->channel_list, channelno);
    if(channeln == NULL){
...
static slot_node* add_slot(slot_node** head, int minor){
    ...
    if(new_slot_node == NULL || data == NULL){
        if(new_slot_node != NULL){kfree(new_slot_node);}
